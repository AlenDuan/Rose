...
tprogress_bar::tprogress_bar(display& disp, twindow& window, twidget& widget, const std::string& message, bool quantify)
...
void tprogress_bar::start_mascot_anim(const SDL_Rect& rect)
{
	...
	if (mascot_anim_id_ == nposm) {
		float_animation* anim = start_window_anim_th(disp_, anim_type_, &mascot_anim_id_);
