...
tprogress::tprogress(display& disp, const std::string& message, const boost::function<bool (tprogress_&)>& did_first_drawn, bool quantify, int hidden_ms, bool show_cancel, int best_width, int best_height)
	: disp_(disp)
	, message_(message)
...
void tprogress::pre_show(CVideo& /*video*/, twindow& window)
{
	...
	window.set_border(null_str);

	progress_.reset(new tprogress_bar(disp_, *window_, find_widget<twidget>(window_, "_progress", false), message_, quantify_));
	progress_->track().set_best_size_1th(track_best_width_, track_best_height_);

	tbutton* button = find_widget<tbutton>(&window, "_cancel", false, false);
	if (!show_cancel_) {
...
bool run_with_progress(display& disp, const std::string& message, const boost::function<bool (tprogress_&)>& did_first_drawn, bool quantify, int hidden_ms, bool show_cancel, const SDL_Rect& rect)
{
	gui2::tprogress dlg(disp, message, did_first_drawn, quantify, hidden_ms, show_cancel, rect.w, rect.h);
	dlg.show(disp.video(), rect.x, rect.y);
