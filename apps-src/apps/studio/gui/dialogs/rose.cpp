...
void trose::export_app(tapp_copier& copier)
{
	...
	if (fok) {
		if (is_studio_app(copier.app)) {
			...
			fok = copier.other_extra_exporter->handle(disp_);
		}
	}
...
void trose::right_click_explorer(ttree_node& node, const tpoint& coordinate)
{
	...
	for (std::vector<std::unique_ptr<tapp_copier> >::const_iterator it = app_copiers.begin(); it != app_copiers.end(); ++ it) {
		...
		sub_items.push_back(gui2::tmenu::titem(vgettext2("Export", symbols) + "...", app_index * app_wight + at2 ++, "misc/export.png"));
		...
		if (!is_reserve_app(app.app)) {
			sub_items.push_back(gui2::tmenu::titem(vgettext2("Remove", symbols) + "...", app_index * app_wight + at2 ++, "misc/remove.png"));
		} else {
			at2 ++;
		}
	...
	int selected;
	{
		gui2::tmenu dlg(disp_, items, nposm);
	...
	if (selected == new_app_at) {
		...
		{
			gui2::tcapabilities dlg(disp_, app_copiers, nposm);
...
*/
		...
		if (selected % app_wight == opencv) {
			fok = opencv_header->handle(disp_);
		}
		symbols["result"] = fok? _("Success"): _("Fail");
		ss.str("");
...
{
	net::fetch_url_data("https://www.github.com", "1.html");
	return;
/*
	{
		treport* report = find_widget<treport>(&window, "toolbar", false, true);
